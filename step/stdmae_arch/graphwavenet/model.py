...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, supports,dropout=0.3, gcn_bool=True, embed=512,addaptadj=True ,
                  t_len = 12, aptinit=None, in_dim=2,out_dim=12,residual_channels=32,
                  dilation_channels=32,skip_channels=256,end_channels=512,kernel_size=2,
                  blocks=4,layers=2, **kwargs):
        ...
        self.fc_his_t = nn.Sequential(nn.Linear(self.embed, 512), nn.ReLU(), nn.Linear(512, 256), nn.ReLU())
        self.fc_his_s = nn.Sequential(nn.Linear(self.embed, 512), nn.ReLU(), nn.Linear(512, 256), nn.ReLU())
