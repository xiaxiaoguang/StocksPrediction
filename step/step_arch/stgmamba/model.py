...
class KFGN(nn.Module):
    def __init__(self, K, feature_size, Clamp_A=True):
        super(KFGN, self).__init__()
        self.feature_size = feature_size
        self.hidden_size = feature_size
        self.K = K
        self.A_list = []
        self.gc_list = nn.ModuleList([DynamicFilterGNN(feature_size, 
                                                       feature_size, 
                                                       bias=False) for i in range(K)])
...
class KFGN_Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        super().__init__()
        self.args = args
        self.kfgn = KFGN(K=args.K, feature_size=args.feature_size)
