...
class DiscreteGraphLearning4(nn.Module):
    """Dynamic graph learning module."""

    def __init__(self, dataset_name, k , input_seq_len,output_seq_len,num_nodes,
                 pls=5 , move_window=5, embed_dim=96 ,num_decoder=3 ,num_token=160 ,num_feats=45):
        super().__init__()
        self.pls=pls
        ...
        self.train_length = {"METR-LA": 23990, "PEMS04": 13599, "PEMS03": 15303, "PEMS07": 16513,\
                              "PEMS-BAY": 36482, "PEMS08": 14284,"StockD":3617, "StockDEBUG":3617, "csi500":4823}[dataset_name]
        
        self.dim_fc = {"METR-LA": 383552, "PEMS04": 217296, "PEMS03": 244560, "PEMS07": 263920,\
                        "PEMS-BAY": 583424, "PEMS08": 228256,"StockD":57600,"StockDEBUG":57600, "csi500":76896}[dataset_name]
        
        self.TARGET_FEATURES = [0]

        # self.node_target = 0
        if dataset_name.startswith("StockD") or dataset_name.startswith("csi"):
            self.TARGET_FEATURES = list(range(45))
            # self.node_target = 1
        self.num_feats = num_feats

        self.node_feats = torch.from_numpy(load_pkl("datasets/" + dataset_name + "/data_in{0}_out{1}.pkl".format(input_seq_len, output_seq_len))["processed_data"]).float()[:self.train_length, :, :]
        
        self.embedding_dim = embed_dim
        ## network structure
        self.time_conv = torch.nn.Conv1d(in_channels=self.embedding_dim,
                                         out_channels=self.embedding_dim,
                                         kernel_size=num_token)
        
        self.conv1 = torch.nn.Conv1d(self.num_feats, 32, move_window, stride=1)  # .to(device)
        self.conv2 = torch.nn.Conv1d(32, 32, move_window, stride=2)  # .to(device)
        ...
        # self.fc_his=torch.nn.Linear(embed_dim+embed_dim,embed_dim,device='cuda:0')
        # self.bn_his=[]
        # for i in range(self.decoder_num):
        #     self.bn_his.append(torch.nn.BatchNorm1d(num_nodes,device='cuda:0'))

        # self.upsampling=Upsampling(patch,embed_dim,num_decoder,needLinear=False)
    ...
    def forward(self, long_term_history, tsformer):
        ...
        global_feat = self.node_feats.to(device).permute(1,2,0)
        # transpose(1, 0).view(num_nodes, self.num_feats, -1)
         # torch.Size([207, 1, 23990]) , 207 是图节点数
        ...
        # with torch.no_grad(): # is this really right ?
        hidden_states,reconstruction_full = tsformer(long_term_history[..., self.TARGET_FEATURES ])
        ...
        # ret_states = hidden_states[:,:,-1,:]
        self.numcount += 1 
        if self.numcount % 100 == 0:
            with open("test3.out",'a') as f:
                print("hidden_states",hidden_states[0,0,:].flatten(),file=f)
                
        return bernoulli_unnorm, reconstruction_full, adj_knn, sampled_adj