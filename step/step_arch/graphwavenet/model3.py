...
class GraphWaveNet3(nn.Module):
    ...
    def __init__(self, num_nodes, support_len, dropout=0.3, gcn_bool=True, t_len = 12,
                  addaptadj=True, aptinit=None,embed_dim2=512, embed_dim=96, in_dim=2 , out_dim=12,
                   residual_channels=32, dilation_channels=32,skip_channels=256,pred_len=12,
        ...
        super(GraphWaveNet3, self).__init__()
        self.dropout = dropout
        self.blocks = blocks
        self.layers = layers
        ...
        self.fc_his = nn.Linear(embed_dim2, embed_dim)
        ...
        self.fc_his2 = nn.Linear(pred_len,out_dim)
    ...
    def forward(self, input, hidden_states, sampled_adj):
        ...
        # hidden_states = self.fc_his(hidden_states).transpose(1,2).unsqueeze(-1)        # B, N, D
        # skip = skip + hidden_states
        hidden_states = self.fc_his2(hidden_states)
        # skip = skip + hidden_states
        x = nn.GELU()(skip)
        x = nn.GELU()(self.end_conv_1(x)) 
        x = self.end_conv_2(x)
        ...
        if self.numcount % 150 == 0:
            with open("test2.out",'a') as f:
                print("output x :",x[0,:,0,0].flatten(),"\n hidden+skip :",skip[0,:,0,0],file=f)
