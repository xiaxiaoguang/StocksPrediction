...
class GraphWaveNet(nn.Module):
    ...
    def forward(self, input, hidden_states, sampled_adj):
        ...
        input = input[:, :2, :, :]
        # torch.Size([32, 12, 207, 3]) -> torch.Size([32, 3, 207, 13]) -> torch.Size([32, 2, 207, 13])
        ...
        x = self.start_conv(x) # torch.Size([32, 32, 207, 13]) 升维度
        skip = 0
        ...
        for i in range(self.blocks * self.layers):
            ...
            residual = x
            # dilated convolution
            filter = self.filter_convs[i](residual) # Conv2d 卷积 1*2 大小和Conv1d是一样的，通道数 出来就是dilation
