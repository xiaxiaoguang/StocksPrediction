...
from Formers.MSCFormer.transformer_layers import TransformerLayers,TransformerLayers2
...
class GraphWaveNet2(nn.Module):
    ...
    def __init__(self, num_nodes, support_len, dropout=0.3, gcn_bool=True, t_len = 12,
        ...
        self.decoder = TransformerLayers2(embed_dim, decoder_depth, 4, 4, dropout)
        self.de_conv_1 = nn.Conv2d(in_channels=num_tokens,out_channels=skip_channels,kernel_size=(1,8),
                                   stride=(1,8), bias=True)
        self.de_conv_2 = nn.Conv2d(in_channels=skip_channels,out_channels=out_dim,kernel_size=(1,embed_dim//8),
                                   stride=(1,embed_dim//8), bias=True)
    ...
    def forward(self, input, hidden_states, sampled_adj):
        ...
        x = self.start_conv(x) # 第一步把feature处理掉，变为只处理目标维度
        skip = 0
        ...
        x = skip
        x = torch.sigmoid(self.end_conv_1(x)) # 尝试这里改为tanh
        x = self.end_conv_2(x)# 最后卷积两次reshape output: [B, P, N, 1] -> [B, N, P]
        x = x.squeeze(-1).transpose(1, 2).unsqueeze(-1)

        hidden_states = hidden_states.permute(0, 2 , 1 ,3)
        hidden_states = torch.relu(self.de_conv_1(hidden_states))        # B, N, D
        hidden_states = self.de_conv_2(hidden_states)        # B, N, D
