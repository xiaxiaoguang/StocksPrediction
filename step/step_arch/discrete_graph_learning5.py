...
class DiscreteGraphLearning5(nn.Module):
    """Dynamic graph learning module."""

    def __init__(self, dataset_name, k , input_seq_len,num_nodes,
                 output_seq_len, pls=5 , move_window=5):
        super().__init__()
        self.pls=pls
        ...
        self.train_length = {"METR-LA": 23990, "PEMS04": 13599, "PEMS03": 15303, "PEMS07": 16513,\
                              "PEMS-BAY": 36482, "PEMS08": 14284,"StockD":3617,"StockDEBUG":3617}[dataset_name]
        ...
        self.dim_fc = {"METR-LA": 383552, "PEMS04": 217296, "PEMS03": 244560, "PEMS07": 263920,\
                        "PEMS-BAY": 583424, "PEMS08": 228256,"StockD":57584,"StockDEBUG":57584}[dataset_name]
    ...
    def forward(self, long_term_history):
        ...
        adj_knn = torch.zeros_like(sampled_adj)
        # breakpoint()
        # sample_adj 就是这个模型算出来的邻接矩阵，然后adj_knn就是根据余弦相似度直接选择的K近邻图！
        return bernoulli_unnorm, _, adj_knn, sampled_adj
