...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, patch_sizeA , patch_sizeB, in_channel, embed_dim, norm_layer, num_feats = 45):
        ...
        self.day_embedding = nn.ModuleList()
        self.day_embedding.append(nn.Conv2d(in_channel,embed_dim,kernel_size=(1,num_feats)))
        self.day_embedding.append(nn.ReLU())
        self.day_embedding.append(nn.Conv2d(embed_dim,embed_dim, kernel_size=(1,1)))
        self.day_embedding.append(nn.BatchNorm2d(embed_dim)) # try InstanceNorm2d / LayerNorm2d
        self.day_embedding.append(nn.ReLU())
    ...
    def forward(self, long_term_history):
        ...
        for layers in self.day_embedding :
             long_term_history = layers(long_term_history)
        day_data = self.norm_layer(long_term_history)
        for layers in self.week_embedding:
                long_term_history = layers(long_term_history)
        week_data = self.norm_layer(long_term_history)

        long_term_history = self.week_activate(long_term_history)
        for layers in self.month_embedding:
             long_term_history = layers(long_term_history)
        month_data = self.norm_layer(long_term_history)

        day_data = day_data.squeeze(-1).view(batch_size, num_nodes, self.output_channel, -1)    
        week_data = week_data.squeeze(-1).view(batch_size, num_nodes, self.output_channel, -1)   
