...
class MaskGenerator(nn.Module):
    ...
    def uniform_rand(self):
        ...
        if self.sort:
            self.masked1, _ = torch.sort(self.masked1)
            self.unmasked1, _ = torch.sort(self.unmasked1)
    ...
    def uniform_rand2(self):
        ...
        mask = torch.randperm(self.num_tokens * self.leverage ,device = self.device)
        mask_len = int(self.num_tokens * self.mask_ratio * self.leverage )
        self.masked1 = mask[:mask_len]
        self.unmasked1 = mask[mask_len:]
    ...
    def forward(self):
        self.unmasked1, self.masked1, self.unmasked2, self.masked2 = self.uniform_rand2()
        return self.unmasked1, self.masked1, self.unmasked2, self.masked2
