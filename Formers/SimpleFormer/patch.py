...
class STPatchEmbedding(nn.Module):
    ...
    def __init__(self, patch_size,node_fusion, embed_dim, 
                 norm_layer,num_feats=45,in_channel=1):
        super().__init__()
        self.output_channel = embed_dim
        self.len_patch = patch_size             # the L 12
        self.output_channel = embed_dim
        ...
        #                                 stride=(1, num_feats))
        self.input_t_embedding = nn.Conv2d(
                                        in_channel,
                                        embed_dim,
                                        kernel_size=(patch_size,num_feats),
                                        stride=(patch_size,num_feats))
        self.input_s_embedding = nn.Conv2d(
                                        in_channel,
                                        embed_dim,
                                        kernel_size=(node_fusion,num_feats),
                                        stride=(node_fusion,num_feats))
        self.norm_layer = norm_layer if norm_layer is not None else nn.Identity()
    ...
    def forward(self, long_term_history):
        ...
        output = long_term_history
        # output = self.input_embedding(long_term_history)
        # output = self.norm_layer(output)
        # output = self.act(output)
        output1 = self.input_t_embedding(output)
        output1 = output1.squeeze(-1).view(batch_size, num_nodes, self.output_channel, -1)    # B, N, d, P
        
        assert output1.shape[-1] == len_time_series / self.len_patch

        output= output.reshape(batch_size,num_nodes,len_time_series,-1).reshape(\
            batch_size*len_time_series,1,num_nodes,-1)

        output2 = self.input_s_embedding(output)
        output2 = output2.squeeze(-1).reshape(batch_size, len_time_series,self.output_channel,-1)
        
        assert output2.shape[-1] == num_nodes / self.node_fusion
