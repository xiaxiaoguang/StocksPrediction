...
class MTSFormer(nn.Module):
    ...
    def __init__(self, patch_sizeA ,leverage , in_channel,
        ...
        self.patch_embedding = PatchEmbedding(patch_sizeA ,leverage , in_channel, embed_dim ,norm_layer=None,num_feats=num_feats)
        ...
        self.encoder1 = TransformerLayers(embed_dim, encoder_depth, mlp_ratio, num_heads, dropout)

        self.encoder2 = TransformerLayers2(embed_dim,decoder_depth,mlp_ratio,num_heads,dropout)
        
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 1, embed_dim))
        
        self.decoder = EasyDecoder(num_token ,embed_dim ,num_token//2)
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        patch1,patch2 = self.positional_encoding(input1=patch1,input2=patch2)
        # mask
        if mask:
        ...
        hidden_states_unmasked2 = self.encoder2(tgt=mask_input2,memory=hidden_states_unmasked1)
    ...
    def decoding(self, hidden_states1, masked_index1 ,hidden_states2,masked_index2):
        ...
        states_full2 = self.output_layer2(states_full2.view(batch_size, num_nodes, -1, self.embed_dim))
        ...
        if self.mode == "pre-train":
            ...
            reconstruction_masked_tokens, label_masked_tokens = \
                self.get_reconstructed_masked_tokens(reconstruction_full, history_data, \
                                                    unmasked_index2, masked_index_2)
            return reconstruction_masked_tokens, label_masked_tokens
        else:
