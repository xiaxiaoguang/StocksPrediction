...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, patch_size, in_channel, embed_dim, norm_layer, num_feats = 45):
        ...
        self.device='cuda:0'
        self.embedding = []
        self.outputlayer = nn.Conv2d(mid_dim,self.output_channel,kernel_size=(1,1),device=self.device)
        
        self.inlayer = nn.Sequential(
                (nn.Conv2d(in_channel, mid_dim,kernel_size=(1,num_feats),device=self.device)),
    ...
    def forward(self, long_term_history):
        ...
        output = []
        long_term_history = self.inlayer(long_term_history)
        for i,layer_patch in enumerate(self.embedding):
            long_term_history = torch.relu(layer_patch(long_term_history))
