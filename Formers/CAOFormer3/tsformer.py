...
class CAOFormer3(nn.Module):
    ...
    def __init__(self, patch_size,node_fusion, 
        ...
        # self.encoder_norm2_ti = nn.LayerNorm(embed_dim2)
        ...
        self.enc_2_enc_emb_ti = nn.Linear(num_token*embed_dim,embed_dim2)
        self.enc_2_enc_emb_sp = nn.Linear(num_nodes*embed_dim,embed_dim2)        

        self.tiencoder1  = TransformerLayers(embed_dim, encoder1_depth[0], mlp_ratio, num_heads, dropout)
        self.tiencoder2 = TransformerLayers(embed_dim2, encoder1_depth[1], mlp_ratio,num_heads, dropout)

        self.spencoder1  = TransformerLayers(embed_dim, encoder2_depth[0], mlp_ratio, num_heads, dropout)
        self.spencoder2 = TransformerLayers(embed_dim2, encoder2_depth[1], mlp_ratio,num_heads, dropout)
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        ti_patches       = self.positional_encoding_t(ti_patches)
        hidden_states_ti = self.tiencoder1(ti_patches)
        hidden_states_ti = self.encoder_norm(hidden_states_ti).view(batch_size, num_nodes, -1)
        hidden_states_ti = self.enc_2_enc_emb_ti(hidden_states_ti)
        hidden_states_ti = self.tiencoder2(hidden_states_ti)
        hidden_states_ti = self.encoder_norm2(hidden_states_ti)

        sp_patches       = self.positional_encoding_s(sp_patches)
        hidden_states_sp = self.spencoder1(sp_patches)
        hidden_states_sp = self.encoder_norm(hidden_states_sp).view(batch_size, tim_len, -1)
        hidden_states_sp = self.enc_2_enc_emb_sp(hidden_states_sp)
        hidden_states_sp = self.spencoder2(hidden_states_sp)
        hidden_states_sp = self.encoder_norm2(hidden_states_sp)
    ...
    def decoding(self, hidden_states_ti,hidden_states_sp):
        ...
        """
        hidden_states_ti = nn.GELU()(self.enc_2_dec_emb(hidden_states_ti))
        hidden_states_sp = nn.GELU()(self.enc_2_dec_emb(hidden_states_sp))
        hidden_states_ti = self.decoder_norm(hidden_states_ti)
        hidden_states_sp = self.decoder_norm(hidden_states_sp)

        pred = self.decoder(memory=hidden_states_sp
                                          ,tgt=hidden_states_ti)
        ...
        if self.mode == "pre-train":
            ...
            if batch_seen is not None and batch_seen % 200 == 0 and batch_seen != 0:
                with open("test-1.out",'a') as f:
                    ...
                    print("weight/grad:",self.enc_2_enc_emb_ti.weight[0,:10],self.enc_2_enc_emb_ti.weight.grad[0,:10],file=f)
            return reconstruction_full, future_data[:,:,:,self.selected_feature],hidden_states_ti,hidden_states_sp
        else:
